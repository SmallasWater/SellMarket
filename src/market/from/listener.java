...
public class listener implements Listener {
    ...
    public static LinkedHashMap<Player,Integer> lastWindow = new LinkedHashMap<>();
        ...
        if((event.getPacket() instanceof ModalFormResponsePacket)){
            ...
            switch (fromId){
                ...
                case create.MESSAGE:
                    ...
                    if(lastWindow.containsKey(player)){
                        SaveWindow.sendWindow(lastWindow.get(player),player);
                        lastWindow.remove(player);
                    }
                    break;
                case create.CHOSE:
                    if("null".equals(data)) {
                        ...
                        if(Integer.parseInt(data) == types.size()){
                            if(lastWindow.containsKey(player)){
                                SaveWindow.sendWindow(lastWindow.get(player),player);
                                lastWindow.remove(player);
                            }
                ...
                case create.TYPES:
                    if("null".equals(data)) {
                        ...
                        if(sMarket.sType.size() == Integer.parseInt(data)){
                            if(lastWindow.containsKey(player)){
                                SaveWindow.sendWindow(lastWindow.get(player),player);
                                lastWindow.remove(player);
                            }
                ...
                case create.SEEK_MENU:
                    if("null".equals(data)) {
                        ...
                        if(sMarket.seekItem.get(player).size() == Integer.parseInt(data)){
                            if(lastWindow.containsKey(player)){
                                SaveWindow.sendWindow(lastWindow.get(player),player);
                                lastWindow.remove(player);
                            }
                ...
                case create.TYPES_SHOW:
                    if("null".equals(data)) {
                        ...
                        if(Tools.getItemsByType(sMarket.clickPos.get(player)).size() == Integer.parseInt(data)){
                            if(lastWindow.containsKey(player)){
                                SaveWindow.sendWindow(lastWindow.get(player),player);
                                lastWindow.remove(player);
                            }
                ...
                case create.UPDATA:
                    if("null".equals(data)) {
                        ...
                        if(!sMarket.handItem.containsKey(player)){
                            return;
                        }
                ...
                case create.ADD_INVENTORY:
                    if("null".equals(data)) {
                        ...
                        if(sMarket.seekItem.size() == Integer.parseInt(data)){
                            if(lastWindow.containsKey(player)){
                                SaveWindow.sendWindow(lastWindow.get(player),player);
                                lastWindow.remove(player);
                            }
    ...
    private void sendMenu(Player player,int data){
        ...
        if("物品市场".equals(name)){
            create.sendTypes(player);
            lastWindow.put(player,create.MENU);
