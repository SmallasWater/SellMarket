...
public class create {
    ...
    /** 菜单*/
    public static final int MENU = 0xAAA001;

    /** 类型*/
    public static final int TYPES = 0xAAA002;

    /** 查找*/
    public static final int SEEK = 0xAAA003;

    /** 查找显示*/
    public static final int SEEK_MENU = 0xAAA004;

    /** 物品设置*/
    public static final int SETTING = 0xAAA005;

    /** 上架物品*/
    public static final int UPDATA = 0xAAA006;


    /** 我的选择*/
    public static final int CHOSE = 0xAAA008;

    /** 类型显示*/
    public static final int TYPES_SHOW = 0xAAA009;

    /** 购买界面*/
    public static final int BUY_MENU = 0xAAA0010;
    ...
    public static void sendSetting(Player player){
        iTypes item = sMarket.getApi().clickItem.get(player);
        ...
        String s = "§l§e商品名称: "+item.getName()+"\n\n"+
                  ...
                  "§l§e商品单价: §c"+(EconomyAPI.getInstance().getMonetaryUnit()+item.getMoney())+"\n";
    ...
    /** 查找列表*/
    public static void sendSeekShow(Player player){
        FormWindowSimple simple = new FormWindowSimple(sMarket.PLUGIN_NAME+"--查询结果","");
        seekSetting setting = sMarket.getApi().seekSetting.get(player);
        ...
        sMarket.getApi().seekItem.put(player,seekItems);
        simple.setContent("共查找到: "+simple.getButtons().size()+" 个");
    ...
    private static ElementButton getButton(iTypes item){
        ...
        ElementButton button = new ElementButton(item.getName()+" §7("+item.getId()+")§r"+"\n单价: "
                +(EconomyAPI.getInstance().getMonetaryUnit())+(item.getMoney())+" 剩余:"+item.getCount()+"§e(点击购买)");
        button.addImage(imageData);
        return button;
    }

    /** 购买页面*/
    public static void sendBuyMenu(Player player){
        ...
        for(int i = 0;i < item.getCount();i++){
            if(i == 0){
                ...
                list.add("§a购买全部 预计花费 "+(EconomyAPI.getInstance().getMonetaryUnit())+(item.getCount() * item.getMoney()));
            }else{
                list.add("§b购买"+i+"个 预计花费 "+(EconomyAPI.getInstance().getMonetaryUnit())+(i * item.getMoney()));
            }

        }
        custom.addElement(new ElementStepSlider("购买数量",list));
        send(player,custom,BUY_MENU);
    }

    /** 商品列表(类型)*/
    public static void sendTypeShow(Player player){
        sType t = sMarket.getApi().clickPos.get(player);
    ...
    /** 商品分类*/
    public static void sendTypes(Player player){
    ...
    public static void sendAddSetting(Player player){
        ...
        Item hand = sMarket.getApi().handItem.get(player);
